        ...
        with self.conn:
            ...
            if result:
                ...
                try:
                    ...
                    print(f"Horas registradas para o serviço {service_name}: {total_hours}h, Custo total: R${total_cost}")
                except ValueError:
...
class HourTrackerApp:
    ...
    def create_widgets(self):
        ...
        service_combobox.grid(row=1, column=1, pady=5, padx=(0, 10))
        ...
        ttk.Label(log_hours_frame, text="Minutos Trabalhados:").grid(row=2, column=2, pady=5)
        minutes_entry = ttk.Entry(log_hours_frame)
        minutes_entry.grid(row=2, column=3, pady=5, padx=(0, 5))

        ttk.Label(log_hours_frame, text="Custo por Hora:").grid(row=3, column=0, pady=5)
        rate_entry = ttk.Entry(log_hours_frame)
        rate_entry.grid(row=3, column=1, pady=5, padx=(0, 5))
    ...
    def update_logs_list(self, logs_listbox):
        ...
        for log in logs:
            ...
            logs_listbox.insert(tk.END, f"ID: {log_id}, Serviço: {log_service}, Custo: R${log_cost}, Data e Hora: {log_date}")
